...
public class ArrayReal extends Variable {
	...
	// for integer
	public ArrayReal(int size, Problem problem, boolean isInteger) {
		problem_ = problem;
		size_ = size;
		array_ = new Double[size_];

		for (int i = 0; i < size_; i++) {
			array_[i] = (double) ThreadLocalRandom.current().nextInt(
					(int) problem_.getLowerLimit(i),
					(int) problem_.getUpperLimit(i) + 1);
		} // for
	} // Constructor
