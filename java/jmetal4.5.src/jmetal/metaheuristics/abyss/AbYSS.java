 ...
 * F. Luna, E. Alba, B. Dorronsoro, J.J. Durillo, A. Beham
...
public class AbYSS extends Algorithm {
	...
	/**
	 * Stores the number of subranges in which each encodings.variable is
	 * divided. Used in the diversification method. By default it takes the
	 * value 4 (see the method <code>initParams</code>).
	 */
	...
	public void initParam() {
		...
		archive_ = new CrowdingArchive(archiveSize_,
		...
		frequency_ = new int[numberOfSubranges_][problem_
				.getNumberOfVariables()];
		reverseFrequency_ = new int[numberOfSubranges_][problem_
				.getNumberOfVariables()];
	...
	/**
	 * Returns a <code>Solution</code> using the diversification generation
	 * method described in the scatter search template.
	 * 
	 * @throws JMException
	...
	public Solution diversificationGeneration() throws JMException,
		...
		for (int i = 0; i < problem_.getNumberOfVariables(); i++) {
			...
			for (int j = 0; j < numberOfSubranges_; j++) {
				reverseFrequency_[j][i] = sumOfFrequencyValues_[i]
						- frequency_[j][i];
			...
			if (sumOfReverseFrequencyValues_[i] == 0) {
				...
				value = PseudoRandom.randInt(0,
						sumOfReverseFrequencyValues_[i] - 1);
				range = 0;
			...
			double low = problem_.getLowerLimit(i) + range
	 ...
	 *            first time; if false, indicates that the reference set has to
	 *            be updated with new solutions
	 * @throws JMException
	 */
	public void referenceSetUpdate(boolean build) throws JMException {
		if (build) { // Build a new reference set
			// STEP 1. Select the p best individuals of P, where p is
			// refSet1Size_.
			...
			for (int i = 0; i < solutionSet_.size(); i++) {
				...
				individual.setDistanceToSolutionSet(distance_
						.distanceToSolutionSetInSolutionSpace(individual,
								refSet1_));
			}
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < solutionSet_.size(); j++) {
					double aux = distance_.distanceBetweenSolutions(
							solutionSet_.get(j), individual);
				...
				for (int j = 0; j < refSet2_.size(); j++) {
					for (int k = 0; k < refSet2_.size(); k++) {
						if (i != j) {
							double aux = distance_.distanceBetweenSolutions(
									refSet2_.get(j), refSet2_.get(k));
							if (aux < refSet2_.get(j)
									.getDistanceToSolutionSet()) {
			...
			for (int i = 0; i < subSet_.size(); i++) {
				individual = (Solution) improvementOperator_.execute(subSet_
						.get(i));
				...
				if (refSet1Test(individual)) { // Update distance of RefSet2
					for (int indSet2 = 0; indSet2 < refSet2_.size(); indSet2++) {
						double aux = distance_.distanceBetweenSolutions(
								individual, refSet2_.get(indSet2));
						if (aux < refSet2_.get(indSet2)
								.getDistanceToSolutionSet()) {
	...
	public boolean refSet2Test(Solution solution) throws JMException {
		...
		if (refSet2_.size() < refSet2Size_) {
			solution.setDistanceToSolutionSet(distance_
					.distanceToSolutionSetInSolutionSpace(solution, refSet1_));
			double aux = distance_.distanceToSolutionSetInSolutionSpace(
					solution, refSet2_);
		...
		solution.setDistanceToSolutionSet(distance_
				.distanceToSolutionSetInSolutionSpace(solution, refSet1_));
		double aux = distance_.distanceToSolutionSetInSolutionSpace(solution,
				refSet2_);
		if (aux < solution.getDistanceToSolutionSet()) {
			solution.setDistanceToSolutionSet(aux);
		}
		...
		if (solution.getDistanceToSolutionSet() < peor) {
			...
			// Update distances in REFSET2
			for (int j = 0; j < refSet2_.size(); j++) {
				aux = distance_.distanceBetweenSolutions(refSet2_.get(j),
						solution);
		...
		for (int i = 0; i < refSet1_.size(); i++) {
			...
			for (int j = i + 1; j < refSet1_.size(); j++) {
				...
				if (!parents[0].isMarked() || !parents[1].isMarked()) {
					...
					offSpring = (Solution[]) crossoverOperator_
							.execute(parents);
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		// STEP 3. Main loop
		int newSolutions = 0;
		CrowdingArchive archive2_ = new CrowdingArchive(archiveSize_,
				problem_.getNumberOfObjectives());
		while (archive2_.size() < 100) {
			while (evaluations_ < maxEvaluations) {
				...
				if (evaluations_ < maxEvaluations) {
					...
					for (int i = 0; i < refSet1_.size(); i++) {
						...
						solution.unMarked();
						solution = (Solution) improvementOperator_
								.execute(solution);
						evaluations_ += improvementOperator_.getEvaluations();
						solutionSet_.add(solution);
					}
			...
			for (int i = 0; i < archive_.size(); i++) {
				if (archive2_.size() == 100)
					...
					archive2_.add(archive2_.size(), archive_.get(i));
			}

			// System.out.println("out");

			// System.out.println(archive2_.size());
		}
