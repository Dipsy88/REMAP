...
public class NSGAII_ReleasePlanning extends Algorithm {
	/**
	 * Constructor
	 * 
	 * @param problem
	 *            Problem to solve
	 */
	public NSGAII_ReleasePlanning(Problem problem) {
		super(problem);
	} // NSGAII
	...
	public SolutionSet execute() throws JMException, ClassNotFoundException {
		...
		while (population.size() < 100) {
			...
			if (newSolution.getNormDistance() < 1.73) {
				evaluations++;
		...
		while (evaluations < maxEvaluations) {
			...
			for (int i = 0; i < (populationSize / 2); i++) {
				if (evaluations < maxEvaluations) {
					...
					if (offSpring[0].getNormDistance() < 1.73) {
						evaluations++;
						offspringPopulation.add(offSpring[0]);
					}
					if (offSpring[1].getNormDistance() < 1.73) {
						evaluations++;
