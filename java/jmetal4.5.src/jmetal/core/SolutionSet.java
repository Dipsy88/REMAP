...
public class SolutionSet implements Serializable {
	...
	/**
	 * Dipesh Write Test Case Writes the decision encodings.variable values of
	 * the <code>Solution</code> solutions objects into the set in a file.
	 * 
	 * @param path
	 *            The output file name
	 */
	...
	// Dipesh write values for testing resource optimization
	public void printValuesToFile(String path) {
		try {
			FileOutputStream fos = new FileOutputStream(path, false);
			OutputStreamWriter osw = new OutputStreamWriter(fos);
			BufferedWriter bw = new BufferedWriter(osw);
			...
			if (size() > 0) {
				for (Solution aSolutionsList_ : solutionsList_) {
					for (Double var : aSolutionsList_.getValues())
						bw.write(var + "\t");
					bw.newLine();
				}
			}
	...
	public void printForMLToFile(String path) {
		try {
			...
			if (size() > 0) {
				int numberOfVariablesFF = solutionsList_.get(0)
						.getDecisionVariables().length;
				;
				...
				for (Solution aSolutionsList_ : solutionsList_) {
					...
					// */
					/*
					 * //values for single objective double sum=0; for (int k =
					 * 0; k < 4; k++){ sum +=
