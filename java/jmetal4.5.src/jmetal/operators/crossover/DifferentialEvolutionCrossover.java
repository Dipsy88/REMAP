...
public class DifferentialEvolutionCrossover extends Crossover {
	...
	/**
	 * DEFAULT_K defines a default K value used in variants current-to-rand/1 and
	 * current-to-best/1
	 */
		...
		if ((DE_Variant_.compareTo("rand/1/bin") == 0) || (DE_Variant_.compareTo("best/1/bin") == 0)) {
			for (int j = 0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					 ...
					 * PseudoRandom.randDouble(0, 1) ; value = xChild.getLowerBound(j) + rnd
					 * *(xParent2.getValue(j) - xChild.getLowerBound(j)) ; } if (value >
					 * xChild.getUpperBound(j)) { double rnd = PseudoRandom.randDouble(0, 1) ; value
