...
//import Algorithm.SolutionAlgorithm;
...
public class BinaryTournament2 extends Selection {
	...
	// Modified to select the solution in random
	public Object execute2(Object object) {
		...
		if (index_ < population.size()) {
			...
			int flag = compareNotStrict(solution1, solution2);
			if (flag == -1)
				return solution1;
			else
				return solution2;
	...
	/*
	 * //Dipesh public Object execute2(Object object) { SolutionSet population =
	 * (SolutionSet)object; if (index_ == 0) //Create the permutation { a_= (new
	 * jmetal.util.PermutationUtility()).intPermutation(population.size()); } //
	 * if(index_>population.size()){ // index_=0; // a_= (new
	 * jmetal.util.PermutationUtility()).intPermutation(population.size()); // }
	 * 
	 * 
	 * Solution solution1; solution1 = population.get(a_[index_]);
	 * 
	 * index_++; return solution1;
	 * 
	 * } // execute
	 */
