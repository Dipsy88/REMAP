...
public class MetricsUtil {
	...
	public double[][] getNormalizedFrontVariableSize(double[][] front,
			double[] maximumValue, double[] minimumValue, int size) {

		double[][] normalizedFront = new double[size][];

		for (int i = 0; i < size; i++) {
			normalizedFront[i] = new double[size];
			for (int j = 0; j < front[i].length; j++) {
				normalizedFront[i][j] = (front[i][j] - minimumValue[j])
						/ (maximumValue[j] - minimumValue[j]);
			}
	 ...
	 * @return A solution set
	 */
	public void readNonDominatedSolutionSet(String path,
	...
	public double[] hvContributions(SolutionSet[] populations) {
		...
		double[] maximumValues = getMaximumValues(
				union.writeObjectivesToMatrix(), numberOfObjectives);
		double[] minimumValues = getMinimumValues(
				union.writeObjectivesToMatrix(), numberOfObjectives);
		...
		for (int i = 0; i < normalizedFront.length; i++) {
			if (frontValues[i].length > 0)
				normalizedFront[i] = getNormalizedFront(frontValues[i],
		...
		for (int i = 0; i < populations.length; i++) {
			if (invertedFront[i + 1].length == 0)
				...
				if (invertedFront[i + 1].length != invertedFront[0].length) {
					...
					// System.out.println(hypervolume.calculateHypervolume(invertedFront[0],
